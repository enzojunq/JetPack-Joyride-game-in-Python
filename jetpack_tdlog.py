...
class Constants:
    def __init__(self):
        ...
        self.nb_min_beam = 10
        self.nb_max_beam = 25
        self.step_beam = self.v
        self.step_max_beam = 2*self.v
        self.gap_btw_beams = 374
        
        self.nb_min_rocket = 2
        self.nb_max_rocket = 4
        self.step_rocket = 2*self.v
        self.step_max_rocket = 4*self.v
        self.distance_warning_rocket = 750
        
        self.laser_x = 50
        self.laser_height = 50
        self.step_laser = self.v
        ...
        self.nb_max_wall = 20
        self.gap_wall = 200
        self.speed_max_wall = self.v/2
        self.step_wall = self.v
        self.step_max_wall = 2*self.v
        self.gap_btw_walls = self.height_window/2
        
        self.character_x = 65
        self.speed_walker = 20
        self.gravity = self.v
        self.step_player = self.v
...
class Beam:
    ...
    def update(self, Counter, _player, constants, time):
        self.x -= constants.step_beam*time
        self.rect[0] = self.x
...
class Rocket:
    ...
    def update(self, _counter, player, constants, time):
        self.x -= constants.step_rocket*time
        self.rect[0] = self.x
...
class Wall:
    def __init__(self, x, y, direction, speed, constants):
    ...
    def update(self, Counter, _player, constants, time):
        self.x -= constants.step_wall*time
        self.upRect[0] = self.x
        self.downRect[0] = self.x
        if ((self.x < constants.character_x) and (self.increase_score)) :
            Counter.update()
            self.increase_score = False
        if ((self.y + self.direction*self.speed*time <= constants.height_window/2 - constants.gap_wall/2 - 2*constants.edge) and
            (self.y + self.direction*self.speed*time >= -(constants.height_window/2 - constants.gap_wall/2 - 2*constants.edge))):
            self.y = self.y + self.direction*self.speed*time
        else:
...
class Laser:
    ...
    def update(self, _counter, _player, constants, time):
        self.x -= constants.step_laser*time
...
class Mode_beam:
    def __init__(self, constants):
        ...
        for i in range(random.randint(constants.nb_min_rocket, constants.nb_max_rocket)):
            new_rocket = Rocket(random.randint(floor(constants.step_rocket/constants.step_beam)*(constants.width_window + constants.distance_warning_rocket), floor(constants.step_rocket/constants.step_beam)*max_x_beam), 0)
        ...
        if (self.next_constants.step_beam < self.next_constants.step_max_beam):
            self.next_constants.step_beam += self.next_constants.step_max_beam/10
        if (self.next_constants.step_rocket < self.next_constants.step_max_rocket):
            self.next_constants.step_rocket += self.next_constants.step_max_rocket/10
    ...
    def __init__(self, constants):
        ...
        for i in range(random.randint(constants.nb_min_wall, constants.nb_max_wall)):
            self.walls.append(Wall(constants.width_window + i*constants.gap_btw_walls,
                                   ...
                                   random.choice([-1, 1]),
                                   random.uniform(0, constants.speed_max_wall),
                                   constants))
        self.obstacles = self.walls
        self.end = False
        self.next_mode = Mode_laser
        self.next_constants = constants
        if (self.next_constants.step_wall < self.next_constants.step_max_wall):
            self.next_constants.step_wall += self.next_constants.step_max_wall/10
...
class Character:
    ...
    def update(self, constants, time):
        if ((self.up) and (self.y - constants.step_player*self.up*time >= constants.edge)):
            self.y -= constants.step_player*self.up*time
            self.up = 0
        elif (self.up):
...
class Game(object):
    def __init__(self):
        ...
        self.mode = Mode_wall(self.constants) 
        self.counter = Counter()
    ...
    def update(self, screen, time_loop):
        keys = check_input()
        ...
        self.check_collisions(time_loop)
